...
void bsp_photodiode_dma_sampling_irq(void)
{
	...
	// Half-transfer
	if (PHOTO_DMA->LISR & DMA_LISR_HTIF1)
	{
		PHOTO_DMA->LIFCR = DMA_LIFCR_CHTIF1;	// Clear Half-transfer flag
	...
	// Transfer-complete
	else if (PHOTO_DMA->LISR & DMA_LISR_TCIF1)
	{
		PHOTO_DMA->LIFCR = DMA_LIFCR_CTCIF1;	// Clear Transfer-complete flag
		...
		if ((photo_diode_adc.block_count) == 0)
		{
		    ...
		    // Xoá cờ ngắt TC/HT cho DMA1 Stream 1
	...
	{
		PHOTO_DMA->LIFCR = DMA_LIFCR_CTEIF1;		// Clear Transfer-error flag
		PHOTO_DMA->LIFCR = DMA_LIFCR_CDMEIF1;		// Clear Direct-mode-error flag
		PHOTO_DMA->LIFCR = DMA_LIFCR_CFEIF1;		// Clear FIFO-error flag
	}
}
